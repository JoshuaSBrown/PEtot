...
c
       ...
       complex*16, allocatable, dimension(:,:) :: h, hh
...
!       read(10,*) i1, nnodes_k,num_group
       read(10,*) i1, nnodes_b,num_group_b,num_group_k
       close(10)
       ...
       if(nnodes_b*num_group_b*num_group_k.ne.nnodes_tot) then
       write(6,*)"nnodes_b*num_group_b*num_group_k.ne.nnodes_tot,stop",
     &  nnodes_b,num_group_b,num_group_k,nnodes_tot
       stop
       endif
...
c        if(itmp.le.num_group_b*num_group_k-2) then
        ...
        natom_dis(1)=itmp*n_tmp1+1       ! Note, for some of them natom_dis(1).gt.natom_dis(2), then no atoms
        natom_dis(2)=natom
        endif
!cccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        if(icolor_b.ge.icolor_tmp) then
           band_dis(1) = icolor_b*nblock + 1
           band_dis(2) = mx
        endif
...
ccccccccccc
       ...
       write(6,*) "************************"
       write(6,*) "  nnodes_b,num_group_b,num_group_k=", 
     &      nnodes_b,num_group_b,num_group_k
       write(6,*) "************************"
       ...
       write(22,*) "************************"
       write(22,3309) nnodes_b,num_group_b,num_group_k  
       write(22,309) natom, ipsp_all
       write(22,301) islda,igga+xgga
       ...
       write(22,*) "************************"
       if(igga.eq.1.and.Ecut.gt.Ecut2/4) then
       write(22,*) "WARNING WARNING WARNING"
       write(22,*) "For igga=1, Ecut2 should be > 4*Ecut,"//
     &  " Otherwise the SCF iteration might not converge." 
       write(22,*) "************************"
       write(6,*) "WARNING WARNING WARNING"
       write(6,*) "For igga=1, Ecut2 should be > 4*Ecut,"//
     &  " Otherwise the SCF iteration might not converge." 
       write(6,*) "************************"
       endif
...
309    format(" natom=",i7,    " ipsp_all=",i7)
3309   format("nnodes_b=",i6,"  num_group_b=",i6,"  num_group_k=",i6)
301    format(" islda=",i7,    "     igga=", f13.10)
...
cccc  ncolx is defined inside fft_allocate
     ...
     &       iflag_fft2L)
       ...
       allocate(ug_n_bp(mg_nx,nblock_band_mx))
...
*********************************************
     ...
     &                     "does not exist, stop"
             ...
             iranm=-4513*(iislda-1)-1371*kpt-5616*inode_b-
     &                           72601*icolor_b
...
ccccccc the write is done by icolor_k.eq.0, not by [kpt_slda_dis(1),kpt_slda_dis(2)]
       ...
       else                     ! icolor_k.eq.0,  only do above for icolor_k.eq.0 due to strange arrangement for iwg_in=1
          do kpt=1,nkpt
...
*********************************************************
     ...
     &    fdens_out,idens_out,nkpt,islda)
       endif
...
*********************************************************
       ...
       call w_line(ilocal,ntype,vwr_atom,Ealpha,ido_DOS)
...
ccccc  fact (1+3.d0*nnodes/(n2L2*n3L2)) is for possible column load imbalance between diff. processors. 
     ...
     &        nnodes,mr_nL2)
       ...
       if (inode_tot==1) write(6,*) 'getvcoul'

       call getvcoul(ntype,Ealpha,icoul) 

       if (inode_tot==1) write(6,*) '1'
...
c       write(22,*) "computational time=", tim
     ...
     & "ave(V_ion+V_Hatree)=0, ave(V_xc).ne.0:  E=E+v0"
       ...
       do kpt=1,nkpt
       write(22,*) "*********************************"
       write(22,104) iislda, kpt,akx(kpt),aky(kpt),akz(kpt)
       ...
       enddo
       write(22,*) "total computation time (sec)=", time11-time00
       write(6,*) "total computation time (sec)=", time11-time00
       call system_flush(22)
       endif
...
cccccccccccc calculated, only the projectors are calculated
      ...
      rewind(23)
      write(23) islda,nkpt,mx,nref_tot,natom,nnodes
      do iislda=1,islda
      do kpt=1,nkpt
      write(23) iislda,kpt,weighkpt(kpt),akx(kpt),aky(kpt),akz(kpt)
      write(23) (E_st(i,kpt,iislda)*27.211396d0,i=1,mx)
      enddo
      enddo
      close(23)
      endif
...
*****************************************************
     ...
     &     (iislda-1)*nkpt+kpt.le.kpt_slda_dis(2)) then
      ...
      do i=1,ngtotnod(inode,kpt)
      ug_n_BP(i,m)=cc*ug_n_tmp(i,m)*fac1+ug_n_BP(i,m)*fac2     !  phase factors corrected
      enddo
      enddo
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  form="unformatted")
       ...
       if(ipsp_all.eq.2.and.ido_DOS.eq.0.
     &   and.ikpt_yno.ne.2) then
       fname9="bpsiiofil"
       kpt1=mod(kpt,10)
       kpt2=mod((kpt-kpt1)/10,10)
       kpt3=mod((kpt-kpt1-kpt2*10)/100,10)
