...
c      complex*16,allocatable,dimension(:,:)  :: ug_n,sug_n, ug_n_bp
      complex*16,allocatable,dimension(:,:)  :: sug_n, ug_n_bp
      ...
      integer nref_tot,mref            ! mref is the max nref of all atoms, replace 32
...
cccc      allocate(ug_n(mg_nx,mx))         ! this should be reduced
      if(ipsp_all.eq.1) then    
      allocate(sug_n(1,mx))            ! dummy
      else
      allocate(sug_n(mg_nx,mx))        ! used for orthogonization, sug_n=S*ug_n, this should be reduced, do it later
      endif
...
cccc if real space NL is used, we don't need wqmask()
      if(ilocal.eq.3) then
      allocate(wqmask(mg_nx,nref_tot))     !  the biggest memory for ilocal.eq.3, projector 
      else
      allocate(wqmask(1,1))
      endif
      allocate(beta_psi(nref_tot,mx/nnodes+1))
      allocate(Dij(mref,mref,natom,islda))
